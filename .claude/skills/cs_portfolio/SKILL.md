...
- `side`: YES or NO (from the position — if the position has `yes` quantity > 0, side is YES; if `no` quantity > 0, side is NO)
- `quantity`: number of contracts held
- `avg_price_cents`: average entry price (from the position's cost / quantity, or from `market_price` field)
