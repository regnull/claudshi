...
argument-hint: "[date]"
allowed-tools: Read Write Bash Glob Grep WebSearch WebFetch mcp__kalshi-mcp__get_balance mcp__kalshi-mcp__get_positions mcp__kalshi-mcp__get_market
---

# /cs_journal — View or Generate Daily Journal Entry

## Usage

```
/cs_journal
/cs_journal <YYYY-MM-DD>
```
...
When the user invokes `/cs_journal`, follow **every** step below in order.
...
```
Invalid date format. Use `/cs_journal YYYY-MM-DD` (e.g., `/cs_journal 2026-04-12`).
```
...
Use `/cs_journal` (no date) to generate today's entry.
```
...
---
*Generated by Claudshi /cs_journal at <timestamp>*
```
...
# If a journal entry already exists for today (e.g., from /cs_monitor), append
existing = ""
...
- **Append, don't overwrite.** If a journal entry already exists for today (e.g., from `/cs_monitor`), append the new content below a separator.
