...
When the user invokes `/cs_monitor`, follow **every** step below in order. Do not skip steps.
...
No tracked markets found. Use `/cs_analyze` to analyze a market or `/cs_scan` to find opportunities.
```
...
For each market where something material changed, perform an **incremental analysis update** (lighter than a full `/cs_analyze`):
...
```python
from memory import (
    write_md, write_yaml, read_yaml, append_yaml_list,
...
---
*Generated by Claudshi /cs_monitor at <timestamp>*
"""
...
<if Reduce/Exit: "Run `/cs_exit <ticker>` to close position">
...
- **Never place trades from this skill.** Only recommend. Tell the user to use `/cs_bet` for new positions and `/cs_exit` for closing.
...
- **Show the interval.** Remind the user when to run `/cs_monitor` next (from `monitor_interval_hours` config, default 12).
